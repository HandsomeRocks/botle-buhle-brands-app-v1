...
bool? searchResults(
  ...
  // I want to compare 1 argument with another
  return searchAgainst.toLowerCase().contains(searchCriteria.toLowerCase());
}

String? checkForNull(String? input) {
  return input?.replaceAll('null', '');
}

String? setLowerCase(String stringValue) {
  // I want to compare 1 argument with another
  return stringValue.substring(0, 1) + stringValue.substring(1).toLowerCase();
}

String? getDialCodeFromMobile(String? mobile) {
  ...
  // Iterate through the country dial codes
  for (var country in countryInfoJson.keys) {
    var dialCode = countryInfoJson[country]!['dial_code'];
    if (dialCode != null) {
      dialCode = dialCode.replaceAll('+', '');
      if (mobile != null && mobile.startsWith(dialCode)) {
        // Return the country dial code
        return '+' + dialCode;
      }
    }
  }
...
String? setColor(String stringValue) {
  // I want to compare 1 argument with another
  return stringValue.toLowerCase();
}

String? cleanProvince(String province) {
  // Remove the brackets from a string like this: Gauteng (ZA)
  return province.replaceAll(RegExp(r'\s*\([^)]*\)\s*'), '');
}

String generateWhatsAppInvitation(
  String dateTime,
  String location,
  String managerName,
  String title,
...
List<String> getStates(String country) {
  // Hardcoded map for the specified countries
  Map<String, List<String>> countryProvinces = {
    'South Africa': [
      'Eastern Cape',
      'Free State',
      'Gauteng',
    ...
    'Lesotho': ['Maseru', 'Leribe', 'Berea', 'Mafeteng'],
    'Eswatini': ['Hhohho', 'Manzini', 'Lubombo', 'Shiselweni'],
    'Mozambique': ['Maputo', 'Nampula', 'Zambezia', 'Tete'],
    'Zimbabwe': ['Harare', 'Bulawayo', 'Mashonaland', 'Manicaland'],
    'Namibia': ['Khomas', 'Erongo', 'Oshana', 'Ohangwena'],
    // Add more countries and their provinces as needed
  };

  return countryProvinces.containsKey(country)
      ? countryProvinces[country]!
      : [];
}

String? countryFlag(String? dialCode) {
  if (dialCode == null) {
    return null;
  }
  final countryInfoJson = {
  ...
  // Find the country name based on the provided dial code
  String? countryName;
  for (var entry in countryInfoJson.entries) {
    if (entry.value['dial_code'] == dialCode) {
...
String getLocation(String? location) {
  return location! + 'Mzee';
}

String friendlyMobile(String mobile) {
  final countryInfoJson = {
    "Afghanistan": {"dial_code": "+93", "code": "AF"},
    "Aland Islands": {"dial_code": "+358", "code": "AX"},
    "Albania": {"dial_code": "+355", "code": "AL"},
  ...
  for (var country in countryInfoJson.keys) {
    ...
    if (dialCode != null) {
      dialCode = dialCode.replaceAll('+', '');
      if (mobile != null && mobile.startsWith(dialCode)) {
        // Remove dial code and add '0'
        return '0' + mobile.substring(dialCode.length);
      }
    }
  }
...
bool validateRSAID(String saidnumber) {
